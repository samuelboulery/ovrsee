...
- The README version badge reads from `package.json` instead of a hardcoded
  number (#95).
- **The seven README screenshots are regenerated by a command** (`pnpm screenshots`,
  T-0264) rather than taken by hand: the app is driven for real through Electron,
  screenmat frames each shot, and a test fails when a tab has no picture — or a
  picture no tab. The old ones dated from before the light theme and still showed
  defects fixed since.
